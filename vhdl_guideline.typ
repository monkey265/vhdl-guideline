...
#ref-table(
  ...
  header: ([*Characteristic*], [*Function*], [*Procedure*]),
  [*Return Value*], [Single value via `RETURN`], [None (modifies `OUT` / `INOUT` parameters)],
  [*Parameter Modes*], [`IN` only (default class `CONSTANT`) \ _(VHDL-2008: `FILE`, `PROTECTED`)_], [`IN`, `OUT`, `INOUT` (classes `CONSTANT`, `VARIABLE`, `SIGNAL`)],
  [*Invocation*], [Within expressions, concurrent assignments, generic maps], [As sequential statement in a process or concurrent procedure call],
...
#note[
Custom functions and procedures *should* use `lowercase` names, while IEEE standard functions and datatypes *must* be in `ALL CAPS`.
]
...
#warning[
*Impure functions must NEVER be used for runtime synthesizable datapath logic.* Reading external signals inside an impure function hides sensitivity dependencies. This causes delta-cycle race conditions and simulation-synthesis mismatches.
]
...
#tip[
Normalizing array parameters with `ALIAS a_vec : ... (vec'LENGTH - 1 DOWNTO 0)` guarantees immunity against ascending (`TO`) vs descending (`DOWNTO`) index discrepancies and non-zero slice offsets.
]
...
- *Why it matters*: High-speed external interfaces (ADCs, DACs, SPI flash, Ethernet PHYs) require strict setup ($t_"su"$), hold ($t_"h"$), and clock-to-output ($t_"co"$) times. Fabric routing adds variable delays that break timing across builds. Setting `IOB = "TRUE"` packs the register into the pin buffer. This eliminates interconnect delay and ensures deterministic timing.
